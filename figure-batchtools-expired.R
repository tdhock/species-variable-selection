...
## check that all fold IDS are the same.
job.table <- getJobTable()
job.table[, test.fold := sapply(prob.pars, "[[", "test.fold")]
job.table[algorithm=="xgboost", {
  list(prop.same=mean(rowMeans(sapply(
    job.id, function(id)loadResult(id)$is.train))%in%c(0,1)))
}, by=list(test.fold)]
...
for(test.set.i in 1:nrow(xgboost1)){
  ...
  for(algo.i in 1:nrow(test.set.algos)){
    ...
    pred.prob.vec <- if(algo.name%in%c("earth", "glmnet")){
